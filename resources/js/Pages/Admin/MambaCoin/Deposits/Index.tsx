...
export default function Index({ deposits, filters }) {
    const [search, setSearch] = useState(filters.search || '');
    const [status, setStatus] = useState(filters.status || '');

    const handleSearch = (value: string) => {
        setSearch(value);
        router.get(route('admin.mambacoin.deposits.index'), 
            { search: value, status }, 
            { preserveState: true }
        );
    };
    ...
    const handleStatusUpdate = (id: number, status: string) => {
        if (confirm(`Are you sure you want to mark this deposit as ${status}?`)) {
            router.put(route('admin.mambacoin.deposits.update', id), {
                status,
                notes: ''
            });
        }
    ...
    const columns = [
        ...
        { header: 'Payment Hash', accessor: 'payment_hash' },
    ...
    return (
        <AdminLayout>
            <div className="px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-8">
                    <DataTable columns={columns} data={deposits.data} />
                </div>
            </div>
        </AdminLayout>
    );
